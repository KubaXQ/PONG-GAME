...
void graAI::ruch_gracz2(float delta)
{
    ...
    float predkosc_AI = 100; // zmieñ dla trudnoœci
    float margines = 5.f;      // ¿eby nie drga³

    if (center_gracza2_y < center_pilki_y && gracz2.getPosition().y + gracz2.getSize().y < 590 && center_pilki_x > 600)
    {
            gracz2.move(sf::Vector2f(0, predkosc_AI * delta));
    }
    else if (center_gracza2_y > center_pilki_y + margines && gracz2.getPosition().y > 10 && center_pilki_x > 600)
    {
            gracz2.move(sf::Vector2f(0, -predkosc_AI * delta));
    }
}
